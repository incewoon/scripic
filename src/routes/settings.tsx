...
import { requestPostNotificationsPermission, setNativeRemindersEnabled, requestMediaPermission, openNotificationSettings, openAppSettings } from "@/plugins/notification-permission";
...
const THEME_PREVIEWS: Record<Theme, { bg: string; swatch1: string; swatch2: string; swatch3: string }> = {
  timeless: {
    bg: "linear-gradient(135deg, oklch(0.96 0.012 85), oklch(0.93 0.018 80))",
    swatch1: "oklch(0.18 0.015 60)", swatch2: "oklch(0.74 0.11 80)", swatch3: "oklch(0.96 0.012 85)",
  },
  minimal: {
    bg: "linear-gradient(135deg, oklch(1 0 0), oklch(0.97 0 0))",
    swatch1: "oklch(0.10 0 0)", swatch2: "oklch(0.55 0 0)", swatch3: "oklch(1 0 0)",
  },
  storyteller: {
    bg: "linear-gradient(135deg, oklch(0.94 0.025 60), oklch(0.90 0.045 50))",
    swatch1: "oklch(0.28 0.045 45)", swatch2: "oklch(0.58 0.14 40)", swatch3: "oklch(0.78 0.10 60)",
  },
};
...
function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-md px-5 pt-[calc(2.5rem+env(safe-area-inset-top))] pb-[max(env(safe-area-inset-bottom),0.75rem)]">
      ...
      <section className="rounded-3xl border border-border/60 bg-card/70 p-5 shadow-[var(--shadow-soft)] mb-4 hidden">
        <div className="flex items-start gap-3 mb-3">
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0" style={{ background: "var(--gradient-warm)" }}>
