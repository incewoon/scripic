...
if (deviceId) return dev:${deviceId}; const appId = req.app?.appId; if (appId) returnapp:${appId};
...
: 0;
console.log([chat] recv rid=${rid} msgs={photos?.length ?? 0} photoBytes={lang} mode=${mode});
...
let chunkCount = 0;
try {
console.log([chat] gemini.connect rid=${rid}); for await (const delta of geminiStreamText(body)) { chunkCount++; if (firstTokenAt == null) { firstTokenAt = Date.now(); console.log([chat] gemini.firstToken rid={firstTokenAt - geminiT0}); } full += delta; if (response?.sendChunk) response.sendChunk({ delta }); } console.log([chat] gemini.done rid={Date.now() - geminiT0} chunks={full.length}); } catch (e: any) { console.error([chat] fail rid={e?.constructor?.name} status={Date.now() - chatT0} msg=${e?.message});
...
full = full ? ${full}\n\n$`{tail}` : tail; } else if (userExplicitFinish) { full = stripWrapSentences(full); const tail = lang === "ko" ? "그럼 지금까지 이야기 나눈 내용으로 앨범을 정리해드릴까요?\n[PROPOSE_FINISH]" : "Shall I put together the album based on what we've shared so far?\n[PROPOSE_FINISH]"; full = full ?${full}\n\nlatex
{tail}` : tail; } else if (assistantSoFar + 1 &gt; totalCap) { full = stripWrapSentences(full); const tail = lang === "ko" ? "이제 앨범으로 정리해드릴게요.\n[READY_TO_FINISH]" : "Let me put this together as your album now.\n[READY_TO_FINISH]"; full = full ? `

{full}\n\nlatex
{tail}` : tail; } else if (willBeLastTurn) { full = stripWrapSentences(full); const tail = lang === "ko" ? "이 정도면 충분히 담을 수 있을 것 같아요. 이대로 앨범으로 정리해드릴까요?\n[PROPOSE_FINISH]" : "I think we have enough now. Shall I put these together into your album?\n[PROPOSE_FINISH]"; full = full ? `
...
console.log([chat] done rid=${rid} totalMs={postReplaced} finalChars=${full.length});
return { text: full };
},
);
...
const rid = typeof ridRaw === "string" && ridRaw.length ? ridRaw.slice(0, 64) : "-";
console.log([album] recv rid=${rid} msgs={photoCount} lang={mode} tone=${tone});
...
if (mode !== "story" && mode !== "journal" && mode !== "summary") {
throw new HttpsError("invalid-argument", invalid mode:${String(mode)}); } if (tone !== "politely" && tone !== "friendly" && tone !== "short") { throw new HttpsError("invalid-argument",invalid tone: ${String(tone)});
}
...
const geminiT0 = Date.now();
try {
console.log([album] gemini.start rid=${rid} validatedMs={rid} elapsedMs=latex
{Date.now() - geminiT0}`); } catch (e: any) { console.error(`[album] fail rid=

{rid} kind={e?.status} elapsedMs={e?.message}); await rollbackDailyCount(); if (e instanceof GeminiUnavailableError) { throw new HttpsError("unavailable", "ai_unavailable", { kind: "ai_unavailable", status: e.status }); } if (e instanceof GeminiQuotaError || e instanceof GeminiRateLimitError) { throw new HttpsError("resource-exhausted", "ai_quota_exhausted", { kind: "ai_quota", status: e.status }); } throw new HttpsError("internal", e?.message ?? "gemini failed"); } const parts = result?.candidates?.[0]?.content?.parts ?? []; const fc = parts.find((p: any) =&gt; p.functionCall)?.functionCall; if (!fc?.args) { await rollbackDailyCount(); console.error([album] fail rid={Date.now() - albumT0}); throw new HttpsError("internal", "gemini did not return album"); } console.log([album] done rid={Date.now() - albumT0}`);
return fc.args;
},
);
